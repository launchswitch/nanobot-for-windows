...
| `tools.exec.shell` | `"auto"` | Shell to use for commands. `"auto"` detects the best available shell (PowerShell on Windows, bash on Unix). Override with `"cmd"`, `"powershell"`, `"pwsh"`, or `"bash"`. Falls back to auto-detection if the chosen shell is not found. |
